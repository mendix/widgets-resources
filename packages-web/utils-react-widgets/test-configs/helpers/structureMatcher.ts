...
export function findDifference(
    expected: ElementStructure | ElementStructure[],
    actual: ElementStructure | ElementStructure[],
    strict: boolean
): string {
    ...
    if (typeof expected === "string" || typeof actual === "string" || expected == null || actual == null) {
        return actual !== expected ? `Expected ${toString(expected)}, but got ${toString(actual)}` : "";
    }
        ...
        for (let i = 0; i < expectedJson.children.length; ++i) {
            ...
            if (childDiff != null) {
                return actualJson.children.length !== expectedJson.children.length
                    ? `${msgPrefix} child ${toString(
                          expectedJson.children[i]
                      )}, but it has only ${actualJson.children.map(toString)}`
                    : childDiff;
            }
        }
    ...
    function toString(structure: ElementStructure | ElementStructure[]): string {
        ...
        return (
            "element " +
            json.type +
            ("className" in json.props ? "." + json.props["className"].trim().replace(/\s+/g, ".") : "")
        );
    }
}
...
export function toElementStructure(node: any): ElementStructure | ElementStructure[] {
    ...
    function elemToStructure(reactNode: ReactElement<any>): ElementStructure {
        ...
        const children: ReactChild[] = Array.isArray(props.children)
            ? props.children
            : props.children
            ? [props.children]
            : [];
