...
describe("StarRating", () => {
    ...
    const renderStarRating = (props: StarRatingProps) => {
        const customStyle: CSSProperties = props.starSize === "custom"
        ? { fontSize: `${props.starSizeCustom}px` }
        : {};
        ...
        fullStar = createElement("span", {
            className: `glyphicon glyphicon-star widget-star-rating-font-${props.starSize} ` +
    ...
    describe("of starsize", () => {
        it("'large', should render with class 'widget-star-rating-font-large'", () => {
            ...
            expect(shallowEmptyStar).toHaveClass("widget-star-rating-font-large");
        });
