...
export const VersionString = z
    .string()
    .min(5, { message: "Must be valid semver string" })
    .refine(
        version => /^\d+\.\d+\.\d+(\.\d+)?$/.test(version),
        version => ({ message: `Unknown version format: "${version}"` })
    );
...
export const versionStringToVersion = VersionString.transform(str => Version.fromString(str));
