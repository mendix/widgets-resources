describe("time-series-chart-web", () => {
    ...
    it("renders time series chart with multiple series and compares with a screenshot baseline", { retries: 3 }, () => {
    ...
    it("renders time series chart without range slider and compares with a screenshot baseline", { retries: 3 }, () => {
    ...
    describe("fill area", () => {
        it(
            ...
            { retries: 3 },
            () => {
                cy.get(".mx-name-containerWithoutFillArea", { timeout: 10000 }).should("be.visible");
        ...
        it("renders time series chart with custom fill area color and compares with a screenshot baseline", () => {
            cy.get(".mx-name-containerCustomFillAreaColor")
                .scrollIntoView()
                .compareSnapshot(`timeSeriesChartCustomFillAreaColor-${browserName}`, 0.1);
        });
    });
    ...
    describe("y axis range", () => {
        ...
        it("renders column chart with auto values and compares with a screenshot baseline", () => {
            cy.get(".mx-name-containerYRangeAuto")
                .scrollIntoView()
                .compareSnapshot(`timeSeriesChartYRangeAuto-${browserName}`, 0.1);
        });
    });
});
