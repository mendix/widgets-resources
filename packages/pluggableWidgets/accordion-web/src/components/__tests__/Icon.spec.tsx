...
describe("Icon", () => {
    ...
    it("renders glyph icons", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} />);

        expect(iconWrapper).toMatchSnapshot();
    });

    it("renders image icons", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} data={{ type: "image", iconUrl: "icon.url" }} />);

        expect(iconWrapper).toMatchSnapshot();
    });

    it("renders a default icon", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} data={undefined} />);

        expect(iconWrapper).toMatchSnapshot();
    });

    it("doesn't render a default icon while loading", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} data={undefined} loading />);

        expect(iconWrapper).toMatchSnapshot();
    });

    it("doesn't render an icon with an unknown icon data type", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} data={{ type: "unknown" } as any} />);

        expect(iconWrapper).toMatchSnapshot();
    });

    it("doesn't apply an animate class to a glyph icon when animate is false", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} animate={false} />);
    ...
    it("doesn't apply an animate class to a default icon when animate is false", () => {
        const iconWrapper = shallow(<Icon {...defaultIconProps} data={undefined} animate={false} />);
